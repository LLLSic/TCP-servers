...
#include <errno.h>
#include <pthread.h> //10.1--epoll is thread safe
...
void* acceptConn(void* arg) {
    ...
    if (cfd == -1) {
        perror("accept");
        free(info);  //6.2.2 这里我设置的continue，不成功就会重新继续监听
        return NULL;
    }
    
    //9.5.1 方法三设置fd的非阻塞属性
    int flag = fcntl(cfd, F_GETFL);
    flag |= O_NONBLOCK;  //字母O大写
    ...
    ev.data.fd = cfd;
    epoll_ctl(info->epfd, EPOLL_CTL_ADD, cfd, &ev); // ev传进去是值传递，可以复用之前的
...
void* communication(void *arg) {
    ...
    SocketInfo *info = (SocketInfo*)arg;
    //接受数据
    char buff[128]; //9.1 一次性最多五个字节 默认LT
    while (1) { //9.3 在ET的情况下
        //9.4 方法二
        ...
        if (len > 0) { //5.1.1 长度大于0表示成功
            ...
            write(STDOUT_FILENO, buff, len); //这里用printf可能会因为没有字符串最后的\0而导致乱码，所以用write写到终端,STDOUT_FILENO == 1,这里写1也可以
            //返回数据
            send(info->fd, buff, len, 0);  //5.2 通信文件描述符、发送的数据缓存(传入，不会被修改)，实际长度，0默认
            //可以加一个判断send
        }
        else if (len == -1 && errno == EAGAIN) { //9.6 读完了，提示Resource temporarily unavaliable的错误
            printf("数据接受完毕\n");
            break;
        }
